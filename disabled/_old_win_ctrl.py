...
class FocusWinRule(MappingRule):
    mapping = {
        "(jump | focus) <win_selector>": Function(focusWin)
    }
    extras = [
        win_selector,
    ]
...
def translateWin(win_selector, position=None, mon_selector=None, _node=None):
    node = _node
...
class TranslateRule(MappingRule):
    mapping = {
        config.lang.translate_win: Function(translateWin)
    }
    extras = [
